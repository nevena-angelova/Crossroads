    ...
    public class TopicsAdminController : BaseController
    {
        ...
        public ActionResult ListTopics(string orderBy, string searchValue, int? page, string searchProperty)
        {
            ...
            if (!String.IsNullOrWhiteSpace(searchValue))
            {
                ...
                {
                    WhereExpression = t => t.User.UserName.ToLower().Contains(searchValue.ToString().ToLower());
                }
        ...
        public ActionResult AdminEditTopic(AdminEditTopicViewModel topic)
        {
            if (topic != null && ModelState.IsValid)
            {
                ...
                if (topic.IsPriority == true)
                {
                    dbTopic.Priority = 1;
                }
                else
                {
                    dbTopic.Priority = null;
                }

                if (topic.RemoveFlags && dbTopic.TopicFlags.Count > 0)
                {
                    dbTopic.Flags = 0;
                    dbTopic.TopicFlags.Clear();
                }
        ...
        public ActionResult AdminDeleteTopic(int id)
        {
            ...
            var topicVoteIds = topic.TopicVotes.Select(v => v.Id).ToList();
